...
export const getTianqi = function (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    let vals = await IceNet.city.find({
        ...
        if (res.data.status == "ok") {
          try {
            ...
            if (date == "今天") {
              ...
              let url = `https://www.lingmx.com/card/index2.html?date=${date.join(
                ","
...
export const chatWithXiaoBing = async function (msg: string) {
  try {
    return await fetch("https://www.bing.com/english/xiaoiceapi?cc=cn&ensearch=0", {
      headers: {
        accept: "*/*",
