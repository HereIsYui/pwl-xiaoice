...
export const ChatCallBack = async function (
  fish: FishPi,
  data: ChatMsg,
  IceNet?: any
) {
  ...
  switch (data.type) {
    ...
    case 3:
      ...
      if (gift.num <= 0) {
        IceNet.sendMsg(
          `@${data.user} 小气鬼${uname},一个${gift.item}都不给我,小冰亲密度${gift.intimacy}`
        )
      } else if (gift.num <= 5) {
        IceNet.sendMsg(
          `@${data.user} 谢谢小气鬼${uname}送的${gift.num}个${gift.item},小冰亲密度+${gift.intimacy}`
        )
      } else {
        IceNet.sendMsg(
          `@${data.user} 谢谢${uname}送的${gift.num}个${gift.item}:heartbeat:小冰亲密度+${gift.intimacy}`
        )
      }
      break
    case 4:
      ...
      for (let i = 0; i < BankList.length; i++) {
        ...
        if (user.length == 0 && BankInfo.access_type == 0) {
          ...
          uRecord.is_success = 1
          uDetail.point = (
            parseInt(uDetail.point) + parseInt(BankInfo.point)
          ).toString()
          if (BankInfo.access_type == 1) {
            uRecord.uId = data.uId
            IceNet.sendMsg(
              `@${BankInfo.user} ,【IceBank-交易通知】:交易积分:${BankInfo.point} \n 交易方式:存 \n 交易单号:${OrderId}`
            )
          } else {
            fish.chat.send(
              BankInfo.user,
              `【IceBank-交易通知】:交易积分:${BankInfo.point} \n 交易方式:存 \n 交易单号:${OrderId}`
            )
          }
